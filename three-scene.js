...
function createAndroidIcons() {
  ...
  for (let i = 0; i < 30; i++) {
    const geometryCreator =
      iconShapes[Math.floor(Math.random() * iconShapes.length)];
...
function animate() {
  ...
  bots.forEach((bot, index) => {
    ...
    bot.mesh.position.y =
      bot.initialY +
      Math.sin(elapsedTime * bot.floatSpeed * scrollEffect + index) *
        bot.verticalFloat *
        (1 + scrollProgress);
    ...
    bot.mesh.position.x =
      Math.cos(bot.orbitAngle) * bot.orbitRadius * (1 + scrollProgress * 0.5);
    bot.mesh.position.z =
      Math.sin(bot.orbitAngle) * bot.orbitRadius * (1 + scrollProgress * 0.5);

    // Tilt based on movement
    bot.mesh.rotation.z =
      Math.sin(elapsedTime * 0.5 + index) * 0.1 * scrollEffect;
    bot.mesh.rotation.x =
      Math.cos(elapsedTime * 0.3 + index) * 0.05 * scrollEffect;
  ...
  shapes.forEach((shape, index) => {
    ...
    shape.mesh.position.y +=
      Math.sin(elapsedTime * shape.floatSpeed * scrollEffect + index) * 0.03;
  ...
  androidIcons.forEach((icon, index) => {
    ...
    icon.mesh.position.x =
      Math.cos(angle) * icon.orbitRadius * (1 + scrollProgress);
    icon.mesh.position.z =
      Math.sin(angle) * icon.orbitRadius * (1 + scrollProgress);
    icon.mesh.position.y +=
      Math.sin(elapsedTime * icon.floatSpeed) * 0.05 * scrollEffect;
  });
